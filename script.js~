...
var youBuyWorkers = function(pl) {
    ...
    var ask = Math.abs(Math.floor(customPrompt(pl.name + ", how many workers would you like to buy? The most you can is " + max,max)));
    if (ask > max) {
        customAlert("Sorry, you don't have enough money");
        youBuyWorkers(pl);
    } else if (isNaN(ask)) {
        customAlert("Sorry, you didn't type in a number");
        youBuyWorkers(pl);
    } else {
        pl.workers += ask;
        pl.money -= ask*20;
        customAlert(pl.name + " bought " + pl.workers + " workers");
    }
};
var buyWorkers = function(ai) {
    ...
    ai.workers += max;
    ai.money -= max*20;
    customAlert(ai.name + " bought " + ai.workers + " workers");
};
...
var getMoneyM = function(pl) {
    ...
    getMoneyW(pl);
    getMoneyW(pl);
    customAlert(pl.name + " has " + pl.money);
    pl.workers = 0;
};
var taxPayer = function() {
    ...
    if (round === 6) {
        ...
        var left = 6 - round;
        customAlert("There are " + left + " days till taxday");
    }
};
...
var beginRound1 = function() {
    if (player1.name === "#player1") {
        player1.name = customPrompt("What is the first player's name?");
    }
...
var beginRound2 = function() {
    if (player1.name === "#player1") {
        player1.name = customPrompt("What is the first player's name?");
        player2.name = customPrompt("What is the second player's name?");
    }
    youBuyWorkers(player1);
...
var beginRound3 = function() {
    if (player1.name === "#player1") {
        player1.name = customPrompt("What is the first player's name?");
        player2.name = customPrompt("What is the second player's name?");
        player3.name = customPrompt("What is the third player's name?");
    }
    youBuyWorkers(player1);
    youBuyWorkers(player2);
...
var beginRound4 = function() {
    if (player1.name === "#player1") {
        ...
        player2.name = customPrompt("What is the second player's name?");
        player3.name = customPrompt("What is the third player's name?");
        player4.name = customPrompt("What is the fourth player's name?");
    }
    youBuyWorkers(player1);
    youBuyWorkers(player2);
    youBuyWorkers(player3);
